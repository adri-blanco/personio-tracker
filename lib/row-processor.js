...
(function (global) {
  ...
  const { waitFor, waitForNewElement, findRowScope, simulateClick, getTimeSegments, resolveRowElements, sleep } = domUtils;
  ...
  async function processRow({ icon, seenInputs, selectors, timeouts, maxRowAncestorLevels, jitterMaxMinutes, dryRun, randomFn }) {
    ...
    // but must never be auto-filled, so bail out before clicking anything.
    if (selectors.TIME_OFF_ICON && row.querySelector(selectors.TIME_OFF_ICON)) {
