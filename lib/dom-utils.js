...
(function (global) {
  ...
  // Determines whether `icon`'s row is a time-off row (vacation, sick leave,
  // etc.). Those rows also carry an alert-icon but must never be auto-filled,
  // so callers use this to exclude them upfront (from counts and from the
  // processing loop) rather than discovering it only after opening the row.
  function isTimeOffRow(icon, selectors, maxLevels) {
    if (!selectors.TIME_OFF_ICON) return false;
    const row = findRowScope(icon, [selectors.TIME_RANGE_CELL], maxLevels);
    return !!(row && row.querySelector(selectors.TIME_OFF_ICON));
  }
